...
static int rt1316_read_prop(struct sdw_peripheral *peripheral)
{
...
static int rt1316_io_init(struct device *dev, struct sdw_peripheral *peripheral)
{
...
static int rt1316_update_status(struct sdw_peripheral *peripheral,
					enum sdw_peripheral_status status)
{
	struct  rt1316_sdw_priv *rt1316 = dev_get_drvdata(&peripheral->dev);
	 ...
	 */
	if (rt1316->hw_init || rt1316->status != SDW_PERIPHERAL_ATTACHED)
		return 0;

	/* perform I/O transfers required for Peripheral initialization */
	return rt1316_io_init(&peripheral->dev, peripheral);
}
	...
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	retval = sdw_stream_add_peripheral(rt1316->sdw_peripheral, &stream_config,
				...
				struct snd_soc_dai *dai)
{
	...
	sdw_stream_remove_peripheral(rt1316->sdw_peripheral, stream->sdw_stream);
	return 0;
}
 ...
 */
static struct sdw_peripheral_ops rt1316_peripheral_ops = {
				...
				struct sdw_peripheral *peripheral)
{
	...
	rt1316->sdw_peripheral = peripheral;
...
static int rt1316_sdw_probe(struct sdw_peripheral *peripheral,
				const struct sdw_device_id *id)
{
	struct regmap *regmap;

	/* Regmap Initialization */
	regmap = devm_regmap_init_sdw(peripheral, &rt1316_sdw_regmap);
	if (IS_ERR(regmap))
		return PTR_ERR(regmap);

	return rt1316_sdw_init(&peripheral->dev, regmap, peripheral);
}

static const struct sdw_device_id rt1316_id[] = {
	SDW_PERIPHERAL_ENTRY_EXT(0x025d, 0x1316, 0x3, 0x1, 0),
	{},
};
