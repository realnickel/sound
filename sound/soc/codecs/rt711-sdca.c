...
static void rt711_sdca_jack_detect_handler(struct work_struct *work)
{
	...
	if ((rt711->jack_type == SND_JACK_HEADSET) &&
		(rt711->scp_sdca_stat2 & SDW_SCP_SDCA_INT_SDCA_8))
...
static void rt711_sdca_jack_init(struct rt711_sdca_priv *rt711)
{
	...
	if (rt711->hs_jack) {
		/* Enable HID1 event & set button RTC as OSC */
		rt711_sdca_index_update_bits(rt711, RT711_VENDOR_HDA_CTL,
			RT711_PUSH_BTN_INT_CTL6, 0x80f0, 0x80f0);
		rt711_sdca_index_update_bits(rt711, RT711_VENDOR_HDA_CTL,
		...
		/* pulse period */
		rt711_sdca_index_update_bits(rt711, RT711_VENDOR_HDA_CTL,
			RT711_PUSH_BTN_INT_CTL9, 0x007f, 0x0004);
		...
		regmap_write(rt711->regmap, SDW_SCP_SDCA_INTMASK2, 0x00);
