/*
 * omap-pcm.c  --  ALSA PCM interface for Haswell
 *
...
{
	struct sst_hsw_pcm *plat_data = dev_get_platdata(platform->dev);
	...
	priv_data->hsw = plat_data->hsw;
	priv_data->sst = plat_data->sst;
...
static int __init sst_haswell_init(void)
{
	return platform_driver_register(&hsw_pcm_driver);
}

static void __exit sst_haswell_exit(void)
{
	platform_driver_unregister(&hsw_pcm_driver);
}

module_init(sst_haswell_init);
module_exit(sst_haswell_exit);

MODULE_AUTHOR("Liam Girdwood, Xingchao Wang");
MODULE_DESCRIPTION("Haswell PCM");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("pci:hsw-pcm");
