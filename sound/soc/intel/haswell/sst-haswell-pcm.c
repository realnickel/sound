			     ...
			     struct snd_pcm_hw_params *params)
{
	...
	dai = mod_map[rtd->old_cpu_dai->id].dai_id;
	...
	if (pcm_data->allocated) {
		...
		pcm_data->stream = sst_hsw_stream_new(hsw, rtd->old_cpu_dai->id,
	...
	ret = create_adsp_page_table(substream, pdata, rtd, runtime->dma_area,
		runtime->dma_bytes, rtd->old_cpu_dai->id);
	if (ret < 0)
		return ret;
	...
	ret = sst_hsw_stream_buffer(hsw, pcm_data->stream,
		pdata->dmab[rtd->old_cpu_dai->id][substream->stream].addr,
		       ...
		       struct snd_soc_pcm_runtime *rtd)
{
	...
	if (pcm->streams[SNDRV_PCM_STREAM_PLAYBACK].substream)
		priv_data->pcm[rtd->old_cpu_dai->id][SNDRV_PCM_STREAM_PLAYBACK].hsw_pcm = pcm;
	if (pcm->streams[SNDRV_PCM_STREAM_CAPTURE].substream)
		priv_data->pcm[rtd->old_cpu_dai->id][SNDRV_PCM_STREAM_CAPTURE].hsw_pcm = pcm;
