...
static int byt_cht_cx2072x_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	struct snd_soc_component *codec = rtd->old_codec_dai->component;
	int ret;
	...
	ret = snd_soc_dai_set_sysclk(rtd->old_codec_dai, CX2072X_MCLK_EXTERNAL_PLL,
	...
	snd_soc_dai_set_bclk_ratio(rtd->old_codec_dai, 50);
				 ...
				 struct snd_pcm_hw_params *params)
{
	 ...
	 */
	ret = snd_soc_dai_set_fmt(rtd->old_cpu_dai,
				SND_SOC_DAIFMT_I2S     |
	...
	ret = snd_soc_dai_set_tdm_slot(rtd->old_cpu_dai, 0x3, 0x3, 2, 24);
	if (ret < 0) {
