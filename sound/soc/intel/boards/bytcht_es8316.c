...
{
	struct snd_soc_component *codec = runtime->old_codec_dai->component;
	...
	ret = snd_soc_dai_set_sysclk(runtime->old_codec_dai, 0, 19200000,
				     SND_SOC_CLOCK_IN);
	if (ret < 0) {
			    ...
			    struct snd_pcm_hw_params *params)
{
	...
	ret = snd_soc_dai_set_tdm_slot(rtd->old_cpu_dai, 0x3, 0x3, 2, bits);
	if (ret < 0) {
