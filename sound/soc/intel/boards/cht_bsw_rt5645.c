...
static int cht_codec_init(struct snd_soc_pcm_runtime *runtime)
{
	...
	struct snd_soc_component *component = runtime->old_codec_dai->component;
	int jack_type;
	int ret;
			    ...
			    struct snd_pcm_hw_params *params)
{
	...
	if ((cht_rt5645_quirk & CHT_RT5645_SSP0_AIF1) ||
		...
		ret = snd_soc_dai_set_fmt(rtd->old_codec_dai,
					SND_SOC_DAIFMT_I2S     |
					SND_SOC_DAIFMT_NB_NF   |
					SND_SOC_DAIFMT_CBS_CFS
			);
		...
		ret = snd_soc_dai_set_tdm_slot(rtd->old_cpu_dai, 0x3, 0x3, 2, 16);
		if (ret < 0) {
		...
		ret = snd_soc_dai_set_tdm_slot(rtd->old_codec_dai, 0xF, 0xF, 4, 24);
		if (ret < 0) {
