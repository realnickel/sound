...
TRACE_EVENT(sdw_config_stream,
	       TP_PROTO(const struct sdw_bus *bus,
			const struct sdw_slave *slv,
			const struct sdw_stream_config *str_cfg,
			const char *stream),
	       ...
	       TP_fast_assign(
		       ...
		       slv ? strncpy(entry->name, dev_name(&slv->dev), 32) :
		       strncpy(entry->name, dev_name(bus->dev), 32);
			      ),
...
TRACE_EVENT(sdw_config_ports,
	       TP_PROTO(const struct sdw_bus *bus,
			const struct sdw_slave *slv,
			const struct sdw_port_config *port_cfg,
			const char *stream),
