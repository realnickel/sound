				      ...
				      bool prepare)
{
	...
	if (prepare) {
		...
		val = sdw_read_nopm(slave->bus, slave->dev_num,
				    SDW_SCP_SYSTEMCTRL);
...
static int sdw_bus_wait_for_clk_prep_deprep(struct sdw_bus *bus, u16 dev_num)
{
	...
	dev_err(bus->dev, "clock stop prep/de-prep failed slave:%d", dev_num);
	return -ETIMEDOUT;
}
...
static int sdw_deprep_clk_stop1(struct sdw_slave *slave)
{
	...
	ret = sdw_read_nopm(slave->bus, slave->dev_num, SDW_SCP_SYSTEMCTRL);
	if (ret <  0) {
		dev_err(slave->bus->dev,
