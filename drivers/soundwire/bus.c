...
sdw_nread_no_pm(struct sdw_peripheral *peripheral, u32 addr, size_t count, u8 *val)
{
	...
	ret = sdw_transfer(peripheral->bus, &msg);
	if (peripheral->is_mockup_device)
		ret = 0;
	return ret;
}
...
int sdw_update_no_pm(struct sdw_peripheral *peripheral, u32 addr, u8 mask, u8 val)
{
	int tmp;
...
int sdw_update(struct sdw_peripheral *peripheral, u32 addr, u8 mask, u8 val)
{
	int tmp;
					   ...
					   bool prepare)
{
	...
	if (prepare) {
		...
		if (ret < 0) {
			if (ret != -ENODATA)
				dev_err(&peripheral->dev, "SDW_SCP_SYSTEMCTRL read failed:%d\n", ret);
			return ret;
		}
		val = ret;
		val &= ~(SDW_SCP_SYSTEMCTRL_CLK_STP_PREP);
	...
	if (ret < 0 && ret != -ENODATA)
		dev_err(&peripheral->dev, "SDW_SCP_SYSTEMCTRL write failed:%d\n", ret);
...
int sdw_bus_prep_clk_stop(struct sdw_bus *bus)
{
	 ...
	 * If one of the Peripheral devices has lost sync and
	...
	list_for_each_entry(peripheral, &bus->peripherals, node) {
		...
		ret = sdw_peripheral_clk_stop_callback(peripheral,
						       SDW_CLK_STOP_MODE0,
						       SDW_CLK_PRE_PREPARE);
		if (ret < 0 && ret != -ENODATA) {
			dev_err(&peripheral->dev, "clock stop pre-prepare cb failed:%d\n", ret);
			return ret;
		}

		/* Only prepare a Peripheral device if needed */
		if (!peripheral->prop.simple_clk_stop_capable) {
			...
			ret = sdw_peripheral_clk_stop_prepare(peripheral,
							      ...
							      true);
			if (ret < 0 && ret != -ENODATA) {
				dev_err(&peripheral->dev, "clock stop prepare failed:%d\n", ret);
				return ret;
			}
		}
	}
	...
	/*
	 * Don't wait for all Peripherals to be ready if they follow the simple
	 * state machine
	 */
	if (!simple_clk_stop) {
		...
		/*
		 * if there are no Peripheral devices present and the reply is
	...
	list_for_each_entry(peripheral, &bus->peripherals, node) {
		...
		if (ret < 0 && ret != -ENODATA) {
			dev_err(&peripheral->dev, "clock stop post-prepare cb failed:%d\n", ret);
			return ret;
		}
	}
...
int sdw_bus_exit_clk_stop(struct sdw_bus *bus)
{
	...
	list_for_each_entry(peripheral, &bus->peripherals, node) {
		...
		ret = sdw_peripheral_clk_stop_callback(peripheral, SDW_CLK_STOP_MODE0,
						       SDW_CLK_PRE_DEPREPARE);
		if (ret < 0)
			dev_warn(&peripheral->dev, "clock stop pre-deprepare cb failed:%d\n", ret);

		/* Only de-prepare a Peripheral device if needed */
		if (!peripheral->prop.simple_clk_stop_capable) {
			simple_clk_stop = false;

			ret = sdw_peripheral_clk_stop_prepare(peripheral, SDW_CLK_STOP_MODE0,
							      false);
...
void sdw_clear_peripheral_status(struct sdw_bus *bus, u32 request)
{
	...
	for (i = 1; i <= SDW_MAX_DEVICES; i++) {
		...
		if (peripheral->status != SDW_PERIPHERAL_UNATTACHED) {
			...
			sdw_update_peripheral_status(peripheral, SDW_PERIPHERAL_UNATTACHED);
		}
