	...
	list_for_each_entry(s_rt, &m_rt->slave_rt_list, m_rt_node) {
		...
		list_for_each_entry(p_rt, &s_rt->port_list, port_node) {
			...
			sdw_fill_xport_params(&p_rt->transport_params,
					p_rt->num, true, SDW_BLK_GRP_CNT_1,
				...
				int port_bo, int hstop)
{
	...
	t_data.hstop = hstop;
	t_data.hstart = hstart = hstop - params->hwidth + 1;
...
static int sdw_add_element_group_count(
			struct sdw_group *group, unsigned int rate)
{
	...
	for (i = 0; i <= num; i++) {
		...
		if (group->count >= group->max_size) {
			...
			group->rates = krealloc(group->rates,
				(sizeof(int) * group->max_size), GFP_KERNEL);
			if (!group->rates)
				return -ENOMEM;
		}
				...
				struct sdw_group *group)
{
	...
	group->rates = kcalloc(group->max_size,
	...
	list_for_each_entry(m_rt, &bus->m_rt_list, bus_node) {
		...
		if (m_rt == list_first_entry(&bus->m_rt_list,
					struct sdw_master_runtime, bus_node)) {
			group->rates[group->count++] = rate;
...
static int sdw_compute_port_params(struct sdw_bus *bus)
{
	...
	params = kzalloc((sizeof(*params) * group.count), GFP_KERNEL);
	if (!params) {
		ret = -ENOMEM;
		goto out;
	}
...
static int sdw_compute_bus_params(struct sdw_bus *bus)
{
	...
	if (mstr_prop->num_clk_gears) {
		...
		is_gear = true;
	} else if (mstr_prop->num_freq) {
		clk_values = mstr_prop->num_freq;
		clk_buf = mstr_prop->freq;
	} else {
		clk_values = 1;
		clk_buf = NULL;
	}

	max_dr_freq = mstr_prop->max_freq * SDW_DOUBLE_RATE_FACTOR;
