...
static int intel_link_power_up(struct sdw_intel *sdw)
{
	...
	if (!*shim_mask) {
		...
		/* FIXME: we would need to mask the actual links */
		spa_mask = SDW_SHIM_LCTL_SPA_MASK;
		cpa_mask = SDW_SHIM_LCTL_CPA_MASK;
...
static int intel_link_power_down(struct sdw_intel *sdw)
{
	...
	*shim_mask &= ~BIT(link_id);
	...
	if (!*shim_mask) {
		...
		/* FIXME: we would need to mask the actual links used */
		spa_mask = (u32)~SDW_SHIM_LCTL_SPA_MASK;
		cpa_mask = (u32)SDW_SHIM_LCTL_CPA_MASK;
...
static int intel_resume_runtime(struct device *dev)
{
	...
	if (clock_stop_quirks & SDW_INTEL_CLK_STOP_TEARDOWN) {
		...
		if (!clock_stop0) {
			 ...
			 */
			sdw_cdns_init(&sdw->cdns, multi_master);
