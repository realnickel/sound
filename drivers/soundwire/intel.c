...
static int intel_master_startup(struct sdw_bus *bus)
{
	...
	if (ret < 0) {
		dev_err(&bus->dev, "cannot enable interrupts\n");
		goto err_init;
	}

	ret = sdw_cdns_exit_reset(&sdw->cdns);
	if (ret < 0) {
		dev_err(&bus->dev, "unable to exit bus reset sequence\n");
		goto err_interrupt;
	}
	...
	if (ret) {
		dev_err(&bus->dev, "DAI registration failed: %d\n", ret);
...
static int intel_master_delete(struct sdw_bus *bus)
{
	struct sdw_intel *sdw;
...
struct sdw_link_ops sdw_intel_link_ops = {
	...
	.add = intel_master_add,
	.startup = intel_master_startup,
	.del = intel_master_delete,
